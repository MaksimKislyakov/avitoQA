...
class TestGetItem:
    ...
    @allure.description(
        "Проверяет, что созданный объект можно получить по его ID через GET /api/1/item/:id"
    )
    ...
    def test_get_existing(self, api_client, created_item):
        ...
        if "T" not in created_at:
            allure.attach(
                f"Некорректный формат даты: {created_at}",
    ...
    @allure.description(
        "Проверяет, что сервер корректно возвращает 404 для несуществующего объявления"
    )
